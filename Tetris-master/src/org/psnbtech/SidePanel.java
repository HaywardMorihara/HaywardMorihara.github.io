 ...
 * 
 * Edited by Nathaniel Morihara, Christin Hardin, and Alex Abad
 *
 */
public class SidePanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		 ...
		 */
		g.setFont(LARGE_FONT);
		g.drawString("Basic Controls", SMALL_INSET, offset += TEXT_STRIDE);//offset = CONTROLS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Arrows - Move", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Space/Up - Rotate", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("P - Pause Game", LARGE_INSET, offset += TEXT_STRIDE);
		
		g.drawString("Give Control to...", SMALL_INSET, offset += TEXT_STRIDE);
		if(tetris.isBart()){g.setColor(Color.GREEN);}else{g.setColor(DRAW_COLOR);}
		g.drawString("Q - Bart", LARGE_INSET, offset += TEXT_STRIDE);
		if(tetris.isBart()){g.setColor(DRAW_COLOR);}else{g.setColor(Color.GREEN);}
		g.drawString("W - Player", LARGE_INSET, offset += TEXT_STRIDE);
		
		g.setColor(DRAW_COLOR);
		g.drawString("Speed", SMALL_INSET, offset += TEXT_STRIDE);
		if(tetris.getSpeedSetting()==0){g.setColor(Color.GREEN);}else{g.setColor(DRAW_COLOR);}
		g.drawString("A - Stepwise", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Z - Next Step", LARGE_INSET+SMALL_INSET, offset += TEXT_STRIDE);
		if(tetris.getSpeedSetting()==1){g.setColor(Color.GREEN);}else{g.setColor(DRAW_COLOR);}
		g.drawString("S - Normal", LARGE_INSET, offset += TEXT_STRIDE);
		if(tetris.getSpeedSetting()==2){g.setColor(Color.GREEN);}else{g.setColor(DRAW_COLOR);}
		g.drawString("D - Fast", LARGE_INSET, offset += TEXT_STRIDE);
		if(tetris.getSpeedSetting()==3){g.setColor(Color.GREEN);}else{g.setColor(DRAW_COLOR);}
		g.drawString("F - SuperSpeed", LARGE_INSET, offset += TEXT_STRIDE);
